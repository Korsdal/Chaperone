...
Tests 364 → 378. Clippy `-D warnings` clean. Serve-versus-refuse is byte-for-byte unchanged: no read that worked before behaves differently, pinned by `classifying_the_bytes_never_serves_them` and the three pre-existing round-trip tests passing untouched.
