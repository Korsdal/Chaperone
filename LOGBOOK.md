...
**What's in flight:** *(nothing — the tree is committed and awaiting a push)*

**Blocker, unchanged:** ⚠️ Claude's git tool context cannot authenticate to GitHub
(`Permission denied (publickey)`); the on-disk key is rejected and `gh` is not
installed. **jok must push.**
...
**Worked on:**
- [x] **Audited the roadmap proposal item-by-item against the tree** (three parallel explore passes: coord auth/TLS/GC, endpoint read path/tools/watcher, CI/examples/leases/docs). Verdicts + `file:line` evidence in `specs/reviewed_roadmap.md` (local); the plan itself rewritten as v2 with v1 preserved beside it. **Item IDs kept stable** so the audit's verdicts still map 1:1.
- [x] **D-037 recorded** — on-prem is the product; cloud deferred on a *market judgment*, not an architectural exclusion (review 2027-02-21). New `product` theme in the decision log. Phase 6 rescoped to linkage-only. Identity stays pluggable.
- [x] **1.3 binary read guardrail.** New `sniff` module: 16 container formats by magic bytes, never extension; OOXML told from a plain zip by the first zip entry's name. The refusal is a tool-level result naming what to read instead. Base64 became **opt-in on read** (`allow_binary`) — it was already opt-in on write. Closed `tool_error`'s wildcard arm so a new error variant can no longer ship with no model-facing advice.
- [x] **1.2 TLS.** Was ~90% already shipped (the audit's finding). Delivered the residue: template defaults to TLS + https, a plaintext warning at every start, a cert **preflight** naming which file is missing, and a scheme/TLS **mismatch** rule.
- [x] **1.1 authenticated control channel.** `shared-secret` mode: one per-deployment token (`endpoint_token`, beside the admin token, reusing its file handling and constant-time verify) **plus** the principal header. A new `Authenticated` extractor closed the nine routes that carried no extractor at all. Endpoint gained `CHAPR_COORD_TOKEN`; the wizard creates the token and puts it in the *users* half of the handover. **Wizard default moved to `shared-secret`** — a new install has no laptops to break, and existing installs are untouched and change via `auth_fallback` (D-031).
- [x] **1.5 macOS** added to `check` *and* the new e2e job (jok's call, over deleting the artifact). I-012 closed.
- [x] **1.4 CI e2e job.** Stands up a coordinator and runs the real smoke suites plus the shipped self-test, three legs; the Windows leg drives a **real SMB share** over UNC. Plus an enforcement step (forged token, bare principal header, unauthenticated `journal/clear` → all 401; `/healthz` open). The MSRV job now runs tests, not just a build.
- [x] **Version → 0.1.2, set by jok.** A patch bump, deliberately: "it stays 0.1.x until it is tested and true."
- [x] **The logbook is now tracked** (jok: "they belong there for open source → open project"), reversing D-033's exclusion of it. Customer identifiers scrubbed first — deployable repo, plugin, scripts and folder names now described by role. Contributors named in `NOTICE` stay named. `CLAUDE.md` stays untracked.
...
- Tests **324 → 364**; clippy `-D warnings` clean throughout.
...
**Open questions:** (1) **`cargo fmt` is still not in CI** — 365 files drift from rustfmt, so adding it means reformatting the tree. Mechanical, but a separate commit and jok's call. (2) **1.4 covers 2 of its 5 named scenarios**: the lease-leak suite needs a fault-injecting proxy that does not exist in the repo, and kill-mid-write/journal-recovery and the Office `~$F` refusal have no example at all. (3) **The e2e job has never executed** — every step is proven locally on Windows, but `New-SmbShare` on a GitHub runner, both POSIX legs, and whether Actions accepts the YAML are unverified until a push. (4) **D-D′ is the gate on Track B** and needs jok + Kristian, not code.

**Next session start from:** **the CI run.** This commit is the first to carry the `e2e` job and the macOS legs, so the first push decides whether 1.4 and 1.5 actually hold — expect SMB share creation and macOS hostname resolution to be where it breaks, and both are one-line fixes in `ci.yml` if so. Then the phase-1 follow-ups in open question (2), and the **D-D′ conversation** before anything in Track B gets designed.
